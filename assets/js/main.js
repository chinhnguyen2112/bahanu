...
$(".box_vxmm_main").hover(
	...
	function () {
		$(this).find("img").attr("src", "/images/home/default.png");
	}
);
...
$(document).click(function (event) {
	$target = $(event.target);
	if (
		!$target.closest(".nav_hover").length &&
		$(".nav_hover").is(":visible") &&
		!$target.closest(".menu_active_hover").length
	) {
		$(".nav_hover").hide(100);
...
$(document).click(function (event) {
	$target = $(event.target);
	if (
		!$target.closest(".header_list_tablet").length &&
		$(".header_list_tablet").is(":visible") &&
		!$target.closest(".header_logo_tablet").length
	) {
