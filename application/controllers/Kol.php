...

    {
        if (check_login()) {
            ...
            if ($data_kol != null && $kol != null) {
                ...
                $money = $amount * $price;
                $check_amount =  $this->Account->get_by_id(['id' => $id, 'zen >=' => $money], 'accounts');
                if ($check_amount != null) {
                    $new_zen = $check_amount['zen'] - $money;
                    if ($new_zen >= 0) {
                        $where_update = [
                            'id' => $id
                        ];
        ...
        if ($type == 1 || $type == 2) {
            ...
            if ($update) {
                if ($type == 1) {
                    ...
                    $data_update = [
                        'zen' => $kol['zen'] + ($thue_kol['price'] * $thue_kol['hours'])
                    ];
                } else {
                    ...
                    $data_update = [
                        'zen' => $user_thue['zen'] + ($thue_kol['price'] * $thue_kol['hours'])
                    ];
                }
                $update_new = $this->Account->update($where_update, $data_update, 'accounts');
                $response = [
